...
base_url = "http://ec2-16-171-28-128.eu-north-1.compute.amazonaws.com:8080/api/embedded"
communication_interval = 5000
user_id = "ilwejkrfhiuy4o3y4ljkblkdj"  # Assuming the user ID is provided

# Initialize API client
api = BrewingSystemAPI(base_url)
...
def main():
    try:
        # Fetch the recipe
        try:
            recipe = api.get_recipe_to_brew(device_serial_number)
        ...
        try:
            start_brewing_response = api.start_brewing(device_serial_number, communication_interval)
        ...
        for step_index, step in enumerate(recipe['step']):
            try:
                ...
                while True:
                    ...
                    brewing_report = create_brewing_report(recipe, step_index, step, current_temp_c, step_start_time, 200)
